...
test <- CopyOfalmond_yield(climate_data)

#use gross_revenue function
test_2 <- gross_revenue(test[4])

#use almond_profit_model function
almond_profit_model(gross_revenue = test_2, almond_yield = test[4])
```
...
site2 = gallons %>% map(~almond_profit_model( 
  gross_revenue = test_2, 
  almond_yield = test, 
  gallons =.x,
  acres = 10, 
  prop_of_loss = 0.25,  
...
# results = water_parms %>% pmap(almond_profit_model, 
#                          gross_revenue = test_2, 
#                          almond_yield = test, 
#                          gallons = 9000,
#                          acres = 10, 
...
# use pmap 
# takes function name and then names of all parameters that don't change
site3 = loss_parms %>% pmap(~almond_profit_model( 
  almond_yield = test[4],
  gross_revenue = test_2,
  gallons = 9000,
  acres = 10, 
...
mean_elect = map_df(site3,`[`) 
# and we can add the parameter values for each run
mean_elect = cbind.data.frame(mean_elect, loss_parms)
...
site2df = data.frame(water = site3)
```
